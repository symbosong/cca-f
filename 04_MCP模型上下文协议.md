...
服务器的实现将放在根项目目录的mcp_server.py文件中。文件里已经有基本的MCP服务器设置和一组存储在内存中的文档集合。

在这个项目中，我们使用的是官方MCP Python SDK（mcp包）。这个SDK只需一行代码就能创建MCP服务器，并且定义工具非常简单。要定义一个工具，只需使用装饰器语法，SDK会自动为我们生成工具的JSON schema。

首先实现读取文档工具。使用 `@mcp.tool` 装饰器，设置名称为 `read_contents`，描述为"读取文档内容并以字符串形式返回"。函数接收 `doc_id` 参数（字符串类型），使用 Pydantic 的 `Field` 添加参数描述。实现逻辑是：如果文档ID不在docs字典中，抛出ValueError；否则返回 `docs[doc_id]`。

然后实现编辑文档工具。名称为 `edit_document`，描述为"通过替换文档内容中的字符串来编辑文档"。函数接收三个参数：`doc_id`（要编辑的文档ID）、`old_string`（要替换的文本，必须精确匹配）和 `new_string`（替换后的新文本）。实现逻辑同样先检查文档是否存在，然后使用字符串的 `replace` 方法进行替换。

使用MCP Python SDK定义工具比手动编写JSON schema定义要容易得多。装饰器和Field类型等会被SDK自动整合，生成相应的JSON schema。
...
服务器已经准备就绪，现在我们将转向MCP客户端的实现。客户端位于根项目目录的mcp_client.py文件中。

MCP客户端由一个单独的类组成。这个类将封装一个叫做客户端会话（client session）的东西。客户端会话是与MCP服务器的实际连接，它是MCP Python SDK的一部分。会话本身需要一些资源清理——当我们关闭程序或不再需要服务器时，需要经过一个清理过程。我已经在MCP客户端类中编写了大量清理代码，这也是为什么这个类存在的原因。用更大的类来管理资源清理是非常常见的做法。

客户端的目的是什么？在我们的代码中，有些时候需要获取工具列表发送给Claude，之后还需要运行Claude请求的工具。为了访问MCP服务器获取工具列表或运行工具，就需要使用MCP客户端。客户端本质上是将服务器的功能暴露给代码库的其他部分。

本视频中我们将实现两个函数：`list_tools` 和 `call_tool`。

实现 `list_tools`：`result = await self.session.list_tools()`，然后 `return result.tools`。通过会话（与MCP服务器的连接）调用内置函数获取服务器实现的所有工具定义列表。

实现 `call_tool`：`return await self.session.call_tool(tool_name, tool_input)`。通过会话调用特定工具，传入工具名称和Claude提供的输入参数。

测试：文件底部有一个测试块，可以直接运行mcp_client.py文件。运行 `uv run mcp_client.py` 后，会启动MCP服务器的副本，获取工具列表并打印结果。可以看到read_doc_contents工具和edit_document工具的定义，包括描述和输入schema。

现在功能已就绪，可以运行CLI应用并让Claude使用这些工具。运行 `uv run main.py`，询问Claude关于report.pdf文档的内容，Claude会使用read_document工具获取文档内容并回答。
...
本视频将介绍MCP服务器中的下一个主要功能——资源（Resources）。我们将在项目中实现另一个特性来帮助理解资源。

我们要实现的功能是：允许用户通过输入@符号加文档名来提及（mention）一个文档。当用户这样做时，会自动获取该文档的内容并将其插入到发送给Claude的提示词中。这个功能有两个方面：输入@符号时自动显示可用文档列表（自动完成），提交包含提及的消息时自动获取文档内容并插入提示词。

这里其实有两个独立的功能需要实现。第一是获取所有文档名列表用于自动完成；第二是获取单个文档的具体内容。为获取这些信息，我们将使用资源。

资源允许MCP服务器向客户端公开一定量的数据。我们通常为每个不同的读操作定义一个资源。在我们的例子中，需要两个资源：一个返回文档名列表，一个返回单个文档的内容。

资源有两种类型：直接资源（Direct/Static）和模板资源（Templated）。直接资源有静态URI，如 `docs://documents`。模板资源的URI中包含一个或多个参数，如 `documents/{doc_id}`，参数会被自动解析并作为关键字参数传递给函数。

实现第一个资源（返回所有文档ID）：使用 `@mcp.resource` 装饰器，URI为 `docs://documents`，mime类型为 `application/json`。函数 `list_docs` 返回 `list(docs.keys())`。MCP SDK会自动将返回值转换为字符串。

实现第二个资源（返回单个文档内容）：URI为 `docs://documents/{doc_id}`（模板资源），mime类型为 `text/plain`。函数 `fetch_doc` 接收 `doc_id` 参数，检查文档是否存在后返回 `docs[doc_id]`。

在MCP Inspector中测试：连接服务器后，Resources部分可以列出静态资源（docs://documents）和资源模板（fetch_doc）。运行静态资源会返回包含JSON文本的消息，运行模板资源需要输入doc_id参数，返回纯文本格式的文档内容。
...
我们要在MCP服务器中关注的最后一个主要功能是提示词（Prompts）。和资源一样，我们将通过在项目中实现一个小功能来理解提示词的用途。

我们将添加斜杠命令（slash commands）支持。例如，输入斜杠时会列出支持的命令，目前只有一个叫"format"的命令。选择format后，会提示输入文档ID。运行命令后，目标是让Claude使用Markdown语法重新格式化文档。

这个功能有一个有趣之处：用户已经可以手动输入类似"将report.pdf文件用markdown语法重新格式化"这样的请求，Claude也能合理完成。那我们为什么还要做这个功能呢？

核心思路是：如果我们作为MCP服务器的作者，能够坐下来编写、测试和评估一个为特定场景（将文档转换为Markdown）定制的高质量提示词，用户会获得更好的结果。这就是MCP服务器中提示词功能的真正目标——提前定义一组经过优化的提示词，专门针对服务器的专长领域。

定义提示词使用 `@mcp.prompt` 装饰器，添加名称和描述。函数返回消息列表。需要从 `mcp.server.fastmcp.prompts` 导入 `base`。

实现format提示词：装饰器设置名称为"format"，描述为"以markdown格式重写文档内容"。函数 `format_document` 接收 `doc_id`（字符串类型）参数。函数内定义一个精心编写的提示词，要求Claude获取指定文档内容并用Markdown语法重写，然后编辑文档保存更新。返回 `[base.UserMessage(content=prompt)]`。

在MCP Inspector中测试：连接服务器，找到Prompts部分，列出所有可用提示词，选择format，输入文档ID（如outlook.pdf），点击Get Prompt，可以看到包含完整提示词的消息列表，其中文档ID已被插入。
